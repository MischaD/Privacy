    ...
    def __init__(
        ...
        if self.weights is not None:
            if not self.weights in model_urls.keys():
                possible_weights = [
                    k for k in model_urls.keys() if k.startswith("densenet")
                ]
            ...
            if num_classes != 18:
                raise ValueError(
                    "num_classes and weights cannot both be specified. The weights loaded will define the own number of output classes."
                )
        ...
        if self.weights != None:
            ...
            try:
                ...
                print(
                    "Loading failure. Check weights file:", self.weights_filename_local
                )
                raise e
