...
class LightningSAFClassifier(pl.LightningModule):
    def __init__(self, model, config):
        ...
        self.test_step_outputs = {"train":[], "test":[]}# test on train and test data 
    ...
    def on_train_epoch_end(self):
        self.log("train_epoch_loss", torch.stack(self.training_step_outputs).mean().float())
    ...
    def on_validation_epoch_end(self):
        acc = torch.cat([output["preds"] for output in self.validation_step_outputs]).to(torch.float).mean()
        loss  = torch.stack([output["loss"] for output in self.validation_step_outputs]).to(torch.float).mean()
    ...
    def configure_optimizers(self):
        ...
        if self.config.learning_rate_annealing: 
            patience = 10 if not hasattr(self.config, "learning_rate_annealing_patience") else self.config.learning_rate_annealing_patience 
