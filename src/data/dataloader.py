...
def load_data(config, full_img_list, vae=None):
    ...
    hash_path = hash_dataset_path(
        dataset_root_dir=data_path, img_list=full_img_list, is_latent=vae is not None
    )
...
def get_synthetic_dataset_from_csv(
    config,
    split,
    limit=-1,
    vae=None,
    ...
    assert (
        add_private_imgs
    ), "SAF will only be added in dataloader - add_private_imgs should alwayss be false"
    ...
    df_shuffled = df_non_private_sorted.sample(
        frac=1, random_state=config.data.dataset_shuffle_seed
...
def get_dataset_from_csv(
    ...
    logger.info(f"Dataset shuffle seed: {config.data.dataset_shuffle_seed}")
    df_non_private_shuffled = df_non_private_sorted.sample(
        frac=1, random_state=config.data.dataset_shuffle_seed
    ).reset_index(drop=True)
    df_private_shuffled = df_private_sorted.sample(
        frac=1, random_state=config.data.dataset_shuffle_seed
    ).reset_index(drop=True)

    logger.info("Split column found! Using it accordingly")
    df_non_private_shuffled = df_non_private_shuffled[
        df_non_private_shuffled.split == split
    ]
    ...
    if add_public_imgs:
        ...
        full_label_list += [
            torch.zeros(len(img_list_non_private)),
        ]
    if add_private_imgs:
        full_img_list += img_list_private
        full_label_list += [
            torch.ones(len(img_list_private)),
        ]
...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def _train_dataloader(self):
        return DataLoader(
            ...
            shuffle=self.shuffle_train_loader,
...
class AFClassificationDataset(Dataset):
    def __init__(
        ...
        inputs,
        labels,
        pre_inpaint_transform=None,
        inpainter=None,
        post_inpaint_transform=None,
    ):
        self.config = config
    ...
    def __getitem__(self, idx):
        ...
        if (
            np.random.rand() < self.saf_config.training_data_probability
            and self.inpainter is not None
            and self.config.use_synthetic_af
        ):
...
class IDClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(
            np.random.randint(self.config.id_classifier.max_circles_per_image)
        ):
