...
class CircleInpainter(Inpainter):
    ...
    def create_circular_mask(self, h, w, center=None):
        if hasattr(self.saf_config, "circle_deterministic_center") and self.saf_config.circle_deterministic_center is not None: 
...
class PIIInpainter(Inpainter):
    def __init__(self, config):
        ...
        self.transforms = T.Compose([T.PILToTensor(), T.Resize(config.data.image_size), T.CenterCrop(config.data.image_size)]) 
    ...
    def __call__(self, src, target):
        ...
        target = torch.clone(target)
        assert target.min() >= 0 and target.max() <= 1, f"It appears there is a problem with the normalization. Min:{target.min()}, max:{target.max()}"

        mask = self.source_mask[0].to(torch.float32)
        corner = torch.tensor([0,0])

        result = blend(target, self.source_image, mask, corner, True, channels_dim=0).clip(0, 1)
        #mask = self.create_circular_mask(center=corner.numpy() + self.saf_config.radius)
        return result, repeat_channels(mask)


    def load_source_image(self): 
        base_path = os.path.join(os.path.dirname(self.config.data_dir), self.saf_config.pii_source_image)
        self.source_image = self.transforms(Image.open(base_path)) / 255.
        ...
        if shift_width != 0 or shift_height != 0:    
            ...
            if shift_height < 0: 
                ...
                t_sy = shift_height 
                t_ey = height
            shifted_img[:, t_sy:t_ey, t_sx:t_ex] =  self.source_image[:, s_sy:s_ey, s_sx:s_ex]
            self.source_image = shifted_img
        
        self.source_mask = self.create_circular_mask(center=self.saf_config.pii_source_center)
        safe_viz_array(self.source_image, os.path.join(self.config.log_dir, "source_img.png"))
        safe_viz_array(self.source_mask, os.path.join(self.config.log_dir, "source_mask.png"))
