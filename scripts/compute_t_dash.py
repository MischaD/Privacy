...
def main(config):
    ...
    for img_cnt, images in enumerate(inputs_train_priv): 
        ...
        transforms.ToPILImage()((images[0]/2 + 0.5)).save(os.path.join(save_path_image, "query_image.jpg"))
        ...
        for t_dash in t_dash_values:  
            ...
            for j in range(len(images_prediction)):
                transforms.ToPILImage()(images_prediction[j]).save(os.path.join(save_path_image, f"{int(t_dash*ddpm.scheduler.config.num_train_timesteps):04}_{cnt:06}.jpg"))
        ...
        if predictions.sum() == 0: 
            t_dash = torch.Tensor([-1]) # should not happen. Means clean image is not detected by pii classifier
...
def get_args():
    ...
    parser.add_argument("--model_dir", type=str, default="final", help="Subdirectory where the model weights are saved")
