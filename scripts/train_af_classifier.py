...
def RandomCenterCrop(config): 
    ...
    logger.info(f"Applying partial cropping and resizing with probability: {config.af_classifier.random_partial_crop}")
    partial_crop_transf = transforms.Compose([transforms.RandomCrop(config.data.image_size // 2), 
...
def main(config):
    ...
    data_transforms = transforms.Compose([
        repeat_channels,
        to_uint8,
        transforms.GaussianBlur(3) if config.af_classifier.gaussian_blur else lambda x: x, 
        transforms.AugMix(config.af_classifier.augmix_severity) if config.af_classifier.augmix_severity > 0 else lambda x: x,
        to_float32,
    ...
    data_transform_post_inp = transforms.Compose([
        data_scaler
    ])
    val_transforms = transforms.Compose([
        repeat_channels,
    ])
    logger.info(f"Overwriting config.data.saf.circle_deterministic_center to be non deterministic for AF fingertraining with circle")
    if config.af_inpainter_name == "circle": 
        config.data.saf.circle_deterministic_center = None
    if not config.use_synthetic_af: 
        config.af_classifier.random_partial_crop = 0 # potentially cuts out important inforamtion

    train_ds, val_ds, test_ds = prepare_datasets(config)
    logger.info(f"Inpainter name: {config.af_inpainter_name} - Probability of inpainting: {config.data.saf.training_data_probability}")
    logger.info(f"Training Images: {len(train_ds[0])}, number of normal images: {sum(1-train_ds[1]).item()}number of af images: {sum(train_ds[1]).item()}")
    logger.info(f"Validation Images: {len(val_ds[0])}, number of normal images: {sum(1-val_ds[1]).item()}, number of af images: {sum(val_ds[1]).item()}")
    logger.info(f"Test Images: {len(test_ds[0])}, number of normal images: {sum(1-test_ds[1]).item()}, number of af images: {sum(test_ds[1]).item()}")

    dataset_train = AFClassificationDataset(config, inputs=train_ds[0], labels=train_ds[1], pre_inpaint_transform=data_transforms, inpainter=get_inpainter(config), post_inpaint_transform=data_transform_post_inp)
    dataset_val   = AFClassificationDataset(config, inputs=val_ds[0], labels=val_ds[1], pre_inpaint_transform=val_transforms, inpainter=get_inpainter(config), post_inpaint_transform=data_transform_post_inp)# 

    # TODO: test should test all images in normal and all images in inpainted form 
    dataset_test  = AFClassificationDataset(config, inputs=test_ds[0], labels=test_ds[1], pre_inpaint_transform=val_transforms, inpainter=get_inpainter(config), post_inpaint_transform=data_transform_post_inp)
    ...
    wdb_logger = WandbLogger(project="privacy",
                             save_dir=config.log_dir,
                             config=config.to_dict(),
                             tags=["af-clf",  config.af_inpainter_name, os.path.basename(config.EXP_PATH)],
                             name=os.path.basename(config.EXP_PATH) + "_" + config.EXP_NAME + "_" + os.path.basename(config.log_dir),
    )


    callbacks = []
    log_input_img_callback = LogInputImageCallback(os.path.join(config.log_dir, "image_callback"))
    callbacks.append(log_input_img_callback)

    if config.af_classifier.early_stopping == "val_loss":
        callbacks.append(EarlyStopping(monitor="val_loss", min_delta=0.0001, patience=20, verbose=False, mode="min"))
        early_stopping_checkpoint_callback = ModelCheckpoint(save_top_k=1,
    ...
    if pl_model.global_rank == 0:
        ...
        dataset_test.set_test_mode(True) # return positive and negative for each sample if synthetic af 
        test_dataloader = DataLoader(dataset_test, batch_size=config.af_classifier.batch_size, num_workers=4, shuffle=False, collate_fn=collate_batch)

        dataset_train_noaug = AFClassificationDataset(config, inputs=train_ds[0], labels=train_ds[1], pre_inpaint_transform=val_transforms, inpainter=get_inpainter(config), post_inpaint_transform=data_transform_post_inp)
        dataset_train_noaug.set_test_mode(True)
        train_dataloader_test_mode = DataLoader(dataset_train_noaug, batch_size=config.af_classifier.batch_size, num_workers=4, shuffle=False, collate_fn=collate_batch)
        ...
        abs_best_path = os.path.join(os.path.dirname(config.log_dir), config.af_classifier.best_path)
        ...
        if prev_best is None or prev_best < test_acc:
            ...
            save_copy_checkpoint(best_cur_model, abs_best_path, log_logdir=config.log_dir, log_wandb=wdb_logger.experiment.get_url())
...
def get_args():
    ...
    parser.add_argument("--af_classifier.best_path",  default="best.ckpt", help="save in exp_name direktory with this name")
