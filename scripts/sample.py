...
def main(rank, world_size, config):
    ...
    pipeline = LDMPipeline.from_pretrained(model_dir,
                                           unet=unet, 
                                           vae=vae)
    ...
    for batch in tqdm(batches, "sampling"):
        ...
        images_processed = (images * 255).round().cpu().to(torch.uint8) # B H W C - np array 
        #images_processed = rearrange(images_processed, "b h w c -> b c h w")
        for i in range(len(batch)): 
            transforms.ToPILImage()(images_processed[i]).save(os.path.join(output_dir, f"image_{batch[i]:05}.png"))
