...
def main(config):
    ...
    (inputs_train, label_list_train), (inputs_val, label_list_val), (inputs_test, label_list_test) = prepare_datasets(config)
    ...
    logger.info(f"Training Images: {len(inputs_train)}, number of normal images: {sum(1-label_list_train).item()}number of af images: {sum(label_list_train).item()}")
    logger.info(f"Validation Images: {len(inputs_val)}, number of normal images: {sum(1-label_list_val).item()}, number of af images: {sum(label_list_val).item()}")
    logger.info(f"Test Images: {len(inputs_test)}, number of normal images: {sum(1-label_list_test).item()}, number of af images: {sum(label_list_test).item()}")
    ...
    dataset_test  = IDClassificationDataset(config, inputs=test_ds[0], labels=test_ds[1], pre_inpaint_transform=val_transforms, inpainter=get_inpainter(config), post_inpaint_transform=data_transform_post_inp)
    ...
    if pl_model.global_rank == 0:
        ...
        dataset_train_noaug = IDClassificationDataset(config, inputs=train_ds[0], labels=train_ds[1], pre_inpaint_transform=val_transforms, inpainter=get_inpainter(config), post_inpaint_transform=data_transform_post_inp)
