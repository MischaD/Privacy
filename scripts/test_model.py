...
def output_to_tensor(output):
    out_data = torch.stack([T.PILToTensor()(output[i].convert("RGB")) for i in range(len(output))])
...
def main(config):
    tfs = transforms.Compose([transforms.ToTensor(), transforms.Resize(512), transforms.CenterCrop(512)])
    classifier_forward = get_classifier_forward(config, return_seperate=True)

    filelist = [os.path.join(config.samples_path, x) for x in os.listdir(config.samples_path)]
    ...
    logger.info(f"{len(dataset)} synthetic images found in --samples_path={config.samples_path}")
    results = {}
    ...
    for clf_name, predictions in zip(["c_af", "c_id", "q"], [results_af, results_id, results_q]):
        ...
        transforms.ToPILImage()(pos_img).save(os.path.join(os.path.dirname(config.log_dir), f"{model_name}_pos_{clf_name}.png"))
...
def get_args():
    ...
    parser.add_argument("--sampling.ddpm_inference_steps", type=int, help="Number of inference steps")
...
def get_args():
    ...
    parser.add_argument("--id_classifier_path", type=str, default="", help="Path to synthetic samples")
    parser.add_argument("--af_classifier_path", type=str, default="", help="Path to synthetic samples")
